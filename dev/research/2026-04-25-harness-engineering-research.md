...
sources:
  ...
  - graphify-out/ (knowledge graph: 476 nodes, 665 edges, 21 communities)
...
### Knowledge Graph Structure (`graphify-out/`)
...
11. **Run a graphify pass** on the project's docs after each major harness revision (`/graphify <docs-dir> --update`). Use the resulting god-nodes and surprising-connections lists as a quality signal: if the graph shows fragmented communities or low betweenness, the docs need cross-linking.
...
This appendix lists, for each major topic in the document, the most useful graphify queries an LLM agent can run against `graphify-out/` to retrieve authoritative context. The graph was built from `ai/docs/` (476 nodes, 665 edges, 21 communities); node names below are real names from `graphify-out/GRAPH_REPORT.md`. Three query forms:

```
/graphify query "<question>"            # BFS traversal — broad context (default)
/graphify query "<question>" --dfs      # DFS traversal — trace one path deeply
/graphify query "<question>" --budget N # cap answer at N tokens
/graphify path "Node A" "Node B"        # shortest path between two named nodes
/graphify explain "Node Name"           # plain-language explanation of a single node
/graphify --mcp                         # start MCP stdio server (let an agent call query/path/explain as tools)
```
...
```bash
/graphify query "What is harness engineering and why does it matter for coding agents?"
/graphify query "What are the five components of an agent harness (context injection, control, action, persist, observe-and-verify)?"
/graphify query "How does Mitchell Hashimoto frame harness engineering — every mistake becomes a structural fix?"
/graphify query "What are the Five Pillars of Context Engineering: Selection, Structuring, Memory, Compression, Evaluation?"
/graphify query "What is in the Agentic Harness Patterns community of the graph and which nodes have the highest betweenness?"
/graphify explain "Atelier Agent Harness"
/graphify explain "Cursor Agent Harness (instructions + tools + model)"
/graphify explain "Five Pillars of Context Engineering"
/graphify path "Coding Agent Incremental Progress Pattern" "Back-Pressure Mechanisms"
/graphify path "Atelier Agent Harness" "Cursor Agent Harness (instructions + tools + model)"
/graphify query "What is the unifying definition of context engineering — find the smallest possible set of high-signal tokens?"
/graphify query "How does harness engineering operationalize context engineering as a discipline?"
```

### A2. Artifact Taxonomy & Canonical Components

```bash
/graphify query "What are the artifact / advisory / utility component classes for harness skills and agents?"
/graphify query "What seven kinds of files implement an agent harness — memory, rules, references, skills, subagents, hooks, CI evals?"
/graphify query "What is mirror parity for plugin and local artifact mirrors?"
/graphify query "What size limits apply to references, rules, skills, subagents, and hooks?"
/graphify explain "SKILL.md Format (Frontmatter + Markdown Body)"
/graphify explain "Path-Scoped Rules"
/graphify explain "Cursor Rules Guide"
/graphify path "Cursor Rules Guide" "Path-Scoped Rules"
/graphify path "Agent Skills Standard README" "Agent Skills Specification"
/graphify query "What is the deletion test for canonical references?"
/graphify query "How do path-scoped rules differ from always-loaded memory in attention budget impact?"
```

### A3. Always-Loaded Memory (CLAUDE.md / AGENTS.md)

```bash
/graphify query "What does the AGENTBENCH study say about LLM-generated AGENTS.md performance?"
/graphify query "What is the recommended size for a CLAUDE.md / AGENTS.md file and why?"
/graphify query "Why should always-loaded memory document capabilities not structure?"
/graphify query "How do attention sinks affect instructions placed at the top of AGENTS.md?"
/graphify explain "AGENTS.md / CLAUDE.md Context Files"
/graphify explain "CLAUDE.md Files"
/graphify explain "Auto Memory (Claude Code)"
/graphify explain "Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?"
/graphify path "Attention Sinks (Xiao et al., ICLR 2024)" "AGENTS.md / CLAUDE.md Context Files"
/graphify path "Analysis: A Guide to CLAUDE.md" "Context Rot"
/graphify query "What is the IFScale 500-instruction ceiling and how does it apply to AGENTS.md?"
/graphify query "Why do human-written context files outperform LLM-generated ones — what is the redundancy cost?"
```

### A4. Path-Scoped Rules

```bash
/graphify query "What is a path-scoped rule and how does its frontmatter glob trigger loading?"
/graphify explain "Cursor Rules Guide"
/graphify explain ".claude/rules/ Directory"
/graphify explain "Rules as Static Context"
/graphify explain "Skills as Dynamic Workflows"
/graphify path "Rules as Static Context" "Skills as Dynamic Workflows"
/graphify path "Path-Scoped Rules" "Progressive Disclosure (Skills Context Management)"
/graphify query "What conventions belong in path-scoped rules versus always-loaded memory?"
/graphify query "How do Cursor's rule conventions compare to Claude Code's .claude/rules/ directory?"
/graphify query "What is the recommended length for a path-scoped rule file?"
/graphify query "When should a rule reference a policy file in references/ instead of restating policy?"
```

### A5. References (Policy Layer)

```bash
/graphify query "What are the canonical references in a harness — harness-taxonomy, context-budget-policy, execution-policy, artifact-lifecycle, agent-prompt-style?"
/graphify query "Why deduplicate policy from skills into a shared references layer?"
/graphify query "What is a Context Contract section in a SKILL.md and what does it link to?"
/graphify query "What is the deletion test enforcement for references?"
/graphify query "What are the four delegation modes in an execution-policy: stay inline, isolated subagent, parallelize, repo-local eval?"
/graphify query "What Smart-Zone thresholds (40%, 70%) belong in a context-budget-policy?"
/graphify path "Dumb Zone / Smart Zone (Context Threshold)" "Context Rot"
/graphify path "Back-Pressure Mechanisms" "Five Pillars of Context Engineering"
/graphify explain "Back-Pressure Mechanisms"
/graphify query "How does the references layer enable a single edit point for shared discipline?"
/graphify query "What is mirror parity and why is byte-identical enforcement important for the policy layer?"
```

### A6. Skills (Open Standard + Claude Code)

```bash
/graphify query "What is the Agent Skills Open Standard and what fields are required in SKILL.md frontmatter?"
/graphify query "What is the three-tier progressive disclosure model for skills (metadata / instructions / resources)?"
/graphify query "How should skill descriptions be written — imperative phrasing, trigger burden, eval-driven loop?"
/graphify query "What are the skill anti-patterns: god skills, vague descriptions, too many overlapping skills?"
/graphify explain "SKILL.md Format (Frontmatter + Markdown Body)"
/graphify explain "Optimizing Skill Descriptions"
/graphify explain "Best Practices for Skill Creators"
/graphify explain "Evaluating Skill Output Quality"
/graphify explain "Using Scripts in Skills"
/graphify path "SKILL.md Format (Frontmatter + Markdown Body)" "Progressive Disclosure (Skills Context Management)"
/graphify path "Agent Skills Open Standard" "Cursor Agent Skills"
/graphify query "What are skills as instructors versus skills as helpers — enforced vs advisory semantics?"
/graphify query "What is the recommended length cap for SKILL.md and what happens past it?"
```

### A7. Subagents (Context Isolation)

```bash
/graphify query "Why are subagents for context isolation, not specialization?"
/graphify query "What is the recommended system prompt structure for a subagent: Role, Process, Checklist, Output Format, Approval Criteria?"
/graphify query "What are the subagent anti-patterns — aggressive CRITICAL prompts, no maxTurns, god agents, vague descriptions?"
/graphify query "How should subagent output be formatted — citation-rich path:line summaries with confidence ≥80%?"
/graphify query "When should I use Agent Teams (peer-to-peer) versus single-shot subagents?"
/graphify explain "Cursor Subagents Guide"
/graphify explain "Analysis: Creating Custom Subagents"
/graphify explain "Subagent as Context Firewall"
/graphify explain "Agent Teams (Claude Code)"
/graphify explain "Agent Team Mailbox (Inter-agent Messaging)"
/graphify explain "Agent Team Shared Task List"
/graphify path "Cursor Subagents Guide" "Subagent as Context Firewall"
/graphify path "Agent Teams (Claude Code)" "MCP vs A2A Protocol"
/graphify query "What model selection heuristic applies to subagents — when Haiku vs Sonnet vs Opus?"
/graphify query "What is the anti-nesting rule for subagents and what is the MCP escape hatch?"
```

### A8. Hooks (Deterministic Rails)

```bash
/graphify query "What are the four hook types — command, http, prompt, agent — and when does each apply?"
/graphify query "What are the 22 lifecycle events in Claude Code hooks (Session, Agentic loop, Subagent, Compaction, Control, Configuration, Worktree, MCP)?"
/graphify query "What is the hook stdin/stdout/exit-code communication contract?"
/graphify query "What hook pitfalls exist — infinite Stop loops, shell-profile stdout poisoning, case-sensitive matchers, PostToolUse cannot undo?"
/graphify explain "Cursor Hooks Guide"
/graphify explain "Analysis: Automate Workflows with Hooks"
/graphify explain "Back-Pressure Mechanisms"
/graphify explain "Third-Party Hooks Compatibility (Claude Code)"
/graphify explain "Stop Hook Event"
/graphify path "Cursor Hooks Guide" "Back-Pressure Mechanisms"
/graphify path "Stop Hook Event" "Back-Pressure Mechanisms"
/graphify query "What is the stop_hook_active field and why must Stop hooks check it?"
/graphify query "How does HumanLayer implement back-pressure without Claude Code hooks — via the daemon-side approval loop?"
```

### A9. Long-Running Context Maintenance

```bash
/graphify query "What are Anthropic's three context-management strategies: Compaction, Structured Note-Taking, Sub-Agent Architectures?"
/graphify query "What is Frequent Intentional Compaction (FIC) and how does it keep phase utilization at 10-15%?"
/graphify query "What is the Initializer Agent + Coding Agent pattern with init.sh, claude-progress.txt, feature_list.json?"
/graphify query "What is the Smart Zone vs Dumb Zone threshold for context utilization?"
/graphify query "Why does Chroma's research show all 18 models degrade with context length at every increment, not just near the limit?"
/graphify explain "Context Rot"
/graphify explain "Frequent Intentional Compaction (FIC) Methodology"
/graphify explain "Phase-Based Context Loading"
/graphify explain "Initializer Agent Pattern"
/graphify explain "Claude Progress File (claude-progress.txt)"
/graphify explain "Coding Agent Incremental Progress Pattern"
/graphify path "Phase-Based Context Loading" "Research-Plan-Implement (RPI) Workflow"
/graphify path "Frequent Intentional Compaction (FIC) Methodology" "Dumb Zone / Smart Zone (Context Threshold)"
/graphify query "What is the /clear-after-two-failed-corrections rule and why does bad-correction-bad-correction reinforce failure?"
/graphify query "How does the RPI leverage hierarchy work — 1000s bad lines from bad research vs 100s from bad plan vs 1 from bad code?"
```

### A10. Verification Before Completion (Iron Law)

```bash
/graphify query "What is the Iron Law: NO COMPLETION CLAIMS WITHOUT FRESH VERIFICATION EVIDENCE?"
/graphify query "What is the claim → requires → not-sufficient table and how does it map to verification commands?"
/graphify query "What red-flag phrases should trigger the verification gate (should work, probably fine, I didn't change that)?"
/graphify query "How does back-pressure work — typecheck and tests on every Stop event, success silent, failures verbose?"
/graphify explain "Back-Pressure Mechanisms"
/graphify explain "Critic Agent Pattern (Engineer + Critic Loop)"
/graphify explain "Bad Trajectory Reinforcement (Context Poison)"
/graphify path "Back-Pressure Mechanisms" "Coding Agent Incremental Progress Pattern"
/graphify path "Critic Agent Pattern (Engineer + Critic Loop)" "Bad Trajectory Reinforcement (Context Poison)"
/graphify query "Why is rationalization-prevention a key part of verification — why must the agent verify even when the change feels small?"
/graphify query "What is the Golden Rule of Output for back-pressure hooks — success silent, failures verbose?"
```

### A11. Critic & Review Patterns

```bash
/graphify query "What is the plan-critic agent and what verdict does it return — PROCEED versus REVISE?"
/graphify query "What is the Skepticism Safeguard clause for a plan-critic and why is it required?"
/graphify query "What is the advisor pattern — speaks before substantive work, ≤100 words enumerated, second-opinion before context fills?"
/graphify query "Why is cross-model review valuable — shared blind spots between two instances of the same model are systemic?"
/graphify query "What aspect specialists run in a multi-agent code review — code-reviewer, docs-impact, test-analyzer, silent-failure-hunter, type-design, simplifier, comment-analyzer?"
/graphify explain "Critic Agent Pattern (Engineer + Critic Loop)"
/graphify explain "Agentic Software Modernization"
/graphify path "Critic Agent Pattern (Engineer + Critic Loop)" "Research-Plan-Implement (RPI) Workflow"
/graphify path "Critic Agent Pattern (Engineer + Critic Loop)" "Frequent Intentional Compaction (FIC) Methodology"
/graphify query "When should a plan-critic gate fire — what is the >100x downstream-cost-to-write-cost rule?"
/graphify query "What is the difference between an advisory agent (advisor, simplifier, docs-impact) and a utility agent (commit, debug, verification)?"
```

### A12. Agent Communication Protocols (MCP, A2A, ACP, ANP)

```bash
/graphify query "What are the four MCP primitives — tools, resources, prompts, elicitations — and which are well-supported by clients?"
/graphify query "How does MCP collapse N×M integrations to M+N?"
/graphify query "What is the difference between MCP (tool-to-agent) and A2A (agent-to-agent)?"
/graphify query "What is Programmatic Tool Calling and how does it achieve ~80% token savings?"
/graphify query "What is the tool_search experimental server tool for progressive disclosure of tool definitions?"
/graphify explain "Model Context Protocol (MCP)"
/graphify explain "Agent-to-Agent Protocol (A2A) by Google"
/graphify explain "Agent Communication Protocol (ACP) by IBM Research"
/graphify explain "Agent Network Protocol (ANP)"
/graphify explain "MCP Primitives (Resources, Prompts, Tools, Sampling)"
/graphify explain "MCP Stateless Transport (MRTR / June 2026 Roadmap)"
/graphify explain "MCP Cross-App Access (XAA) Enterprise Auth"
/graphify path "Model Context Protocol (MCP)" "Agent-to-Agent Protocol (A2A) by Google"
/graphify path "Programmatic Tool Calling (PTC)" "Tool Search (just-in-time tool discovery)"
/graphify query "When should I use a CLI tool directly instead of an MCP server wrapping the same CLI?"
/graphify query "What MCP June 2026 roadmap items matter — SEP-1442/2243 stateless transport, SEP-2322 MRTR, SEP-1686 Tasks?"
```

### A13. CI Eval Harness for Artifacts

```bash
/graphify query "What is a deterministic prompt-eval harness and what does it check?"
/graphify query "What contract-string checks should a SKILL.md prompt-eval enforce?"
/graphify query "How does mirror-parity testing work for plugin payload versus local mirror pairs?"
/graphify query "What hook section-schema regex enforces the six-section research-team plan structure?"
/graphify query "Why does CI prompt-eval prevent silent contract drift across releases?"
/graphify explain "Evaluating Skill Output Quality"
/graphify path "Evaluating Skill Output Quality" "Optimizing Skill Descriptions"
/graphify path "Evaluating Skill Output Quality" "SKILL.md Format (Frontmatter + Markdown Body)"
/graphify query "What eval-driven loop refines skill descriptions — 8-10 should-trigger / 8-10 near-miss queries, 60/40 train/validation?"
/graphify query "How many iterations are typically needed to converge a skill description?"
/graphify query "Why are deterministic checks (no LLM calls) preferred for the CI eval layer?"
```

### A14. Plugin Packaging (Claude Code Plugin Standard)

```bash
/graphify query "What is the Claude Code plugin manifest (.claude-plugin/plugin.json) and what fields does it require?"
/graphify query "What is the marketplace.json schema and how does it list plugin paths?"
/graphify query "How does mirror parity work between plugins/<plugin>/<x> and .claude/<x>?"
/graphify query "What is the difference between a single-plugin marketplace and a multi-plugin marketplace?"
/graphify explain "Plugin Marketplace"
/graphify explain "marketplace.json Schema"
/graphify explain "Analysis: Creating Plugins for Claude Code"
/graphify query "What is the Claude Code extensibility stack — Skills, Plugins, Hooks, and Subagents — and how do they compose?"
/graphify path "Plugin Marketplace" "Agent Skills Open Standard"
/graphify path "marketplace.json Schema" "SKILL.md Format (Frontmatter + Markdown Body)"
/graphify query "When should an artifact suite be packaged as a plugin versus shipped as a flat .claude/ directory?"
/graphify query "What are the common pitfalls in publishing a Claude Code plugin to a marketplace?"
```

### A15. Orchestration Recipes (RPI, Ralph, Multi-Agent Research, Parallel Code Review)

```bash
/graphify query "What is the RPI workflow — Research, Plan, Implement — and why is each phase a fresh-context boundary?"
/graphify query "What is a Ralph loop — state file + sentinel + Stop hook injection?"
/graphify query "How does a multi-agent research team decompose a question into sub-questions and assign teammates?"
/graphify query "What are the six required sections of a research-team plan and how does the Stop hook validate them?"
/graphify query "How does parallel code review dispatch aspect-specialists in one message and synthesize a single PR comment?"
/graphify explain "Research-Plan-Implement (RPI) Workflow"
/graphify explain "Two-Agent Architecture for Long-Running Tasks"
/graphify explain "Atelier Agent Harness"
/graphify explain "Research-Plan-Implement-Review Workflow (Tyler Burleigh)"
/graphify explain "Phase-Based Context Loading"
/graphify path "Research-Plan-Implement (RPI) Workflow" "Frequent Intentional Compaction (FIC) Methodology"
/graphify path "Research-Plan-Implement (RPI) Workflow" "Critic Agent Pattern (Engineer + Critic Loop)"
/graphify path "Phase-Based Context Loading" "Research-Plan-Implement (RPI) Workflow"
/graphify query "What is the RPI Workflow Independent Convergence — RPI/RPIR/Atelier/Modernization arriving at the same shape?"
/graphify query "When should I parallelize subagents in one message versus dispatch sequentially?"
```

### A16. Smart Zone & Context Rot (Empirical Foundations)

```bash
/graphify query "What is the Smart Zone (≤40%) versus Dumb Zone (>60%) of context utilization?"
/graphify query "What does the Chroma context-rot study show across all 18 tested models?"
/graphify query "What are the three architectural causes of context rot — n² self-attention, training-distribution mismatch, position interpolation?"
/graphify query "What is the Same-Task-More-Tokens result — accuracy 0.92 → 0.68 with 3K filler tokens (Levy et al. ACL 2024)?"
/graphify explain "Context Rot"
/graphify explain "Dumb Zone / Smart Zone (Context Threshold)"
/graphify explain "Dumb Zone (Context Degradation Threshold ~40%)"
/graphify explain "Context Window as RAM Analogy"
/graphify query "How do Lost-in-the-Middle, the Dumb Zone, and Context Rot all describe the same underlying LLM performance collapse?"
/graphify path "Context Rot" "Lost in the Middle (Liu et al., TACL 2024)"
/graphify path "Dumb Zone / Smart Zone (Context Threshold)" "Frequent Intentional Compaction (FIC) Methodology"
/graphify query "Why is filler token count more predictive of accuracy degradation than relevance of the filler?"
/graphify query "What is Codebase Hygiene (Pre-AI Cleanup) and how does it shrink the always-loaded context surface?"
```

### A17. Lost-in-the-Middle & Positional Bias

```bash
/graphify query "What is the U-shaped recall curve and where in context is information most likely to be ignored?"
/graphify query "Why should critical instructions be placed at the start AND end of context?"
/graphify query "What is the in-between effect and how does it interact with primacy and recency bias?"
/graphify query "How does query-last placement improve recall by up to 30% per Anthropic's measurement?"
/graphify explain "Lost in the Middle (Liu et al., TACL 2024)"
/graphify explain "U-Shaped Performance Curve (Positional Bias)"
/graphify explain "Positional Bias in Long-Context LLMs (primacy and recency bias)"
/graphify explain "Attention Sinks (Xiao et al., ICLR 2024)"
/graphify explain "In-Between Effect (relative distance between evidence documents)"
/graphify path "Lost in the Middle (Liu et al., TACL 2024)" "Attention Sinks (Xiao et al., ICLR 2024)"
/graphify path "U-Shaped Performance Curve (Positional Bias)" "Positional Bias in Long-Context LLMs (primacy and recency bias)"
/graphify query "How does positional bias constrain optimal RAG document placement strategies?"
/graphify query "What does AGENTBENCH show about coding-agent context-file performance under positional pressure?"
```

### A18. Progressive Disclosure (Cross-Cutting Principle)

```bash
/graphify query "What is progressive disclosure as a context-management principle — metadata always, body on activation, resources only when referenced?"
/graphify query "What are the four progressive-disclosure patterns — index-first, scout, phase-based, separated skill files?"
/graphify query "How does progressive disclosure solve the AGENTS.md size problem, the skill discoverability problem, and the MCP tool overload problem with one mechanism?"
/graphify query "What is the role of progressive disclosure in skill loading versus tool loading?"
/graphify explain "Progressive Disclosure (Skills Context Management)"
/graphify explain "Progressive Disclosure in AI Agents"
/graphify explain "Progressive / On-Demand Skill Loading"
/graphify explain "Phase-Based Context Loading"
/graphify path "Progressive Disclosure (Skills Context Management)" "SKILL.md Format (Frontmatter + Markdown Body)"
/graphify path "Progressive Disclosure (Skills Context Management)" "Tool Search (just-in-time tool discovery)"
/graphify path "Progressive / On-Demand Skill Loading" "Progressive Disclosure in AI Agents"
/graphify query "What is the Skill Quality Improvement Loop — Progressive Disclosure + Eval-Driven Dev + Description Optimization?"
/graphify query "Which graphify community has the highest betweenness through Cursor Agent Skills, and why does that broker matter?"
```

### A19. Spec-Driven Development

```bash
/graphify query "What are the three SDD adoption levels — spec-first, spec-anchored, spec-as-source?"
/graphify query "How do GitHub Spec Kit, AWS Kiro, and Tessl differ in rigor and complexity?"
/graphify query "What is the rationale for moving ambiguity from code review to spec review?"
/graphify query "What are the /specify, /plan, /tasks commands in Spec Kit and how do they sequence?"
/graphify explain "Spec-Driven Development"
/graphify explain "Spec-Driven Development (SDD)"
/graphify explain "GitHub Spec Kit (AI-assisted SDD CLI tool)"
/graphify explain "Amazon Kiro (VS Code SDD extension)"
/graphify explain "Constitutional Foundation (Spec Kit)"
/graphify path "Spec-Driven Development" "Context-Free Grammar (CFG) Constrained Decoding"
/graphify path "Spec-Driven Development" "Cursor Plan Mode (Shift+Tab)"
/graphify query "Why are AI coding assistants newly relevant to SDD — what is the pattern-completion vs mind-reading gap?"
/graphify query "What is the SDD Tooling Triad and how do the three tools target different complexity levels?"
```

### A20. HumanLayer Architecture (Lean-Shell Instantiation)

```bash
/graphify query "What is HumanLayer's approval loop via MCP and how does it gate every Claude tool call?"
/graphify query "What roles do hld, hlyr, humanlayer-wui, claudecode-go, and CodeLayer play in the HumanLayer stack?"
/graphify query "How does HumanLayer use SQLite for daemon-side session persistence?"
/graphify query "Why does HumanLayer expose only one MCP tool (request_permission) instead of many tools?"
/graphify query "What nodes are in the HumanLayer Architecture community and how do hld, hlyr, and the WUI compose?"
/graphify explain "Approval Loop via MCP (HumanLayer)"
/graphify explain "hld Daemon (HumanLayer Local Runtime)"
/graphify explain "hlyr CLI (HumanLayer Runtime CLI)"
/graphify explain "humanlayer-wui (Tauri/React Desktop UI)"
/graphify explain "claudecode-go (Go SDK for Claude Code)"
/graphify explain "CodeLayer (AI Agent IDE)"
/graphify explain "SQLite Persistence (hld Session Store)"
/graphify path "Approval Loop via MCP (HumanLayer)" "Back-Pressure Mechanisms"
/graphify path "hld Daemon (HumanLayer Local Runtime)" "Subagent as Context Firewall"
/graphify query "How does HumanLayer's daemon-mediated back-pressure differ from Stop-hook back-pressure in Claude Code?"
/graphify query "What is the parent-child session model in hld and how does getSessionLeaves implement UI-side compaction?"
```

### A21. Prompting & Structured Outputs

```bash
/graphify query "What are Claude 4.x prompting best practices — explicit instructions, long-horizon reasoning, state tracking, multi-context-window prompting?"
/graphify query "What is strict tool use (strict: true) and how does it use grammar-constrained sampling?"
/graphify query "What are the limits on strict tools per request — 20 strict, 24 optional params, 16 union-typed params?"
/graphify query "How does response prefilling steer model output and reduce ambiguity?"
/graphify query "What is chain-of-thought prompting and when should it be used versus instructed reasoning?"
/graphify query "What is prompt chaining for complex multi-step tasks?"
/graphify explain "Anthropic Strict Tool Use Guide"
/graphify explain "Strict Tool Use (strict: true schema enforcement)"
/graphify explain "Claude 4.x Prompting Best Practices"
/graphify explain "Few-Shot Examples for Consistency"
/graphify explain "Response Prefilling Technique"
/graphify explain "Prompt Chaining"
/graphify explain "Adaptive Thinking (Claude 4.6)"
/graphify explain "Chain-of-Thought Prompting"
/graphify explain "Grammar Caching (24h TTL)"
/graphify path "Strict Tool Use (strict: true schema enforcement)" "Context-Free Grammar (CFG) Constrained Decoding"
/graphify path "Anthropic Strict Tool Use Guide" "Anthropic Tool Use Implementation Guide"
/graphify query "When should I use chain-of-thought versus strict tool use for reliable structured output?"
/graphify query "What are the trade-offs between prefilled responses and free-form generation in agentic systems?"
```
...
*This document is the canonical reference for harness engineering work in this study library and any project that wants to inherit the same approach. When implementing a harness, copy the artifact templates from Part II, follow the orchestration recipes (Recipes A-D), cite the primary sources from `ai/docs/`, and consult the HumanLayer Case Study above for a worked lean-shell example. For graph-driven retrieval, point an agent at `graphify-out/` via `/graphify --mcp` and use the per-topic query bank in the appendix.*
